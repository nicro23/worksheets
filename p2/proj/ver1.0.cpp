...
void move_katio(katio& kat, user usr)
{
    if(kat.alive == 1 && kat.cd < 0) {
        if (usr.r == kat.r + 1 && usr.c > kat.c_min && usr.c < kat.c_max) {
            if (usr.c > kat.c_min && usr.c < kat.c_max) {
                if (usr.c > kat.c) {
                    kat.c++;
                } else {
                    kat.c--;
...
void animation(char x[][350], matrix &mat, user &usr,platfrm &platform,bullts &bullet, ldder ladder, hill hills[], int ct_hills, lva lava, katio& kat, cest& chest,ofu&ufo)
{
    //clear the matrix
    clear(x);
...
void die(char x[][350], matrix &mat, user &usr, platfrm &platform,bullts &bullet, ldder ladder, hill hills[], int ct_hills, lva lava , katio &kat, cest& chest,ofu&ufo)
{
    if (mat.level == 0)
    {
        usr.r = 23;
...
//must copy animation refrences[contains animation]
void gravity_and_jump(char x[][350], matrix &mat, user& usr, platfrm &platform, bullts& bullet, ldder ladder, hill hills[], int ct_hills,lva lava, katio &kat, cest& chest,ofu&ufo)
{
    // bynzl kam cell;
...
//must copy animation refrences[contains animation]
void user_action(char x[][350], char dir, user& usr, matrix &mat, platfrm& platform, bullts& bullet, ldder ladder, hill hills[], int ct_hills,lva lava, katio& kat, cest& chest,ofu&ufo)
{
    ...
    */
    ...
    while (true)
    {
        ...
        while (!_kbhit())
        {
            ...
            draw_hero(x, usr);
            //gravity
            gravity_and_jump(x, mat, usr, platform, bullet, ladder, hills, ct_hills, lava, kat, chest,ufo);
            move_elevator(x, usr, platform);
            object_detection(usr, platform, ladder, hills, ct_hills);
            check_death(usr, lava, kat, ufo);
            cd(usr, bullet, platform, kat, ufo);
            if(usr.die == 1)
            {
                animation(x, mat, usr, platform, bullet, ladder, hills, ct_hills, lava, kat, chest,ufo);
                map(x,mat);
                die(x, mat,  usr, platform, bullet,  ladder,  hills,  ct_hills, lava ,  kat, chest,ufo);
            }
            //screen the matrix
            map(x, mat);
        }
        ...
        user_action(x, move, usr, mat, platform, bullet, ladder, hills, ct_hills, lava, kat, chest,ufo);

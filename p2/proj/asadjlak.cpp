...
void move_elevator(char x[][498], user& usr, platfrm &platform)
{
...
{
    int is_left_on_platform = 0, is_right_on_platform = 0, is_hero_on_platform = 0, can_right = 1, can_left = 1, can_up = 0, can_down = 0;
    /*for (int i = usr.r; i < usr.r + 6; i++)
    {
        for (int j = usr.c - 2; j < usr.c + 3; j++)
        {
    ...
    //check platforms
    for (int i = platform.c; i < platform.width + platform.c; i++)
    {
        if (usr.c - 3 == i)
        {
...
void gravity_and_jump(char x[][498], matrix mat, user& usr, enemy enemy1, platfrm platform, int bullets_pos[][2], int ct_bullet, ldder ladder, hill hills[], int ct_hills,lva lava) 
{
    ...
    if (usr.is_jumping == -1)
    {
        for (int i = 0; i < usr.jump_height_ct && usr.is_jumping == -1; i++)
        {
            usr.r--;
            animation(x, mat, usr, enemy1, platform, bullets_pos, ct_bullet, ladder, hills, ct_hills, lava);
            map(x, mat);
    ...
    if (usr.is_jumping == -2)
    {
        for (int i = 0; i < usr.jump_height_ct && usr.is_jumping == -2; i++)
        {
            usr.r--;
            usr.c++;
            animation(x, mat, usr, enemy1, platform, bullets_pos, ct_bullet, ladder, hills, ct_hills, lava);
    ...
    if (usr.is_jumping == -3)
    {
        for (int i = 0; i < usr.jump_height_ct && usr.is_jumping == -3; i++)
        {
            usr.r--;
            usr.c--;
            animation(x, mat, usr, enemy1, platform, bullets_pos, ct_bullet, ladder, hills, ct_hills, lava);
...
void user_action(char x[][498], char dir, user& usr, matrix &mat, enemy &enemy1, platfrm platform, int bullets_pos[][2], int& ct_bullet, ldder ladder, hill hills[], int ct_hills,lva lava)
{
    ...
    if (dir == 'i' && usr.is_jumping == 0 && usr.is_falling == 0)
    {
        usr.is_jumping = -1;
    }
    if (dir == 'l' && usr.is_jumping == 0 && usr.is_falling == 0)
    {
        usr.is_jumping = -2;
    }
    if (dir == 'j' && usr.is_jumping == 0 && usr.is_falling == 0)
    {
        usr.is_jumping = -3;
    }
    //dodge button

...
void mahmoud_fire(char x[][350], mahmod& mahmoud, user usr, matrix mat)
{
...
void draw_bullets(char x[][350], bullts bullet, user usr, matrix mat)
{
    //gets the gun's row {subject to change}
    int gun_r = usr.r + 2;
    //gets the gun's col {subject to change}
...
void move_bullets(char x[][350], bullts& bullet, katio& kat, mahmod& mahmoud, matrix mat)
{
...
//        map(x, mat);
    ...
    if (mat.level == 1)
    {
        ...
        mat.c = 230;
        clear(x);
        //to avoid screen flickering during transition
//        animation(x, mat, usr, platform, bullet, ladder, hills, ct_hills, lava, kat, chest, ufo, mahmoud);
//        map(x, mat);
    }
    usr.die = 0;
...
void user_action(char x[][350], char dir, user& usr, matrix& mat, platfrm& platform, bullts& bullet, ldder ladder, hill hills[], int ct_hills, lva lava, katio& kat, cest& chest, ofu& ufo,mahmod& mahmoud)
{
    ...
    if (dir == 'e' && usr.is_facing_right == 1)
    {
        if (bullet.cd < 1)
        {
            bullet.ct++;
...
//            x[gun_r + 1][gun_c ] = '/';
    ...
    if (dir == 'e' && usr.is_facing_right == 0)
    {
        if (bullet.cd < 1)
        {
            bullet.ct++;
